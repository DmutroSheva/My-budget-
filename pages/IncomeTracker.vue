<template>
  <div class="tracker">
    <h2>Доходы</h2>
    <form
      @submit.prevent="addIncomeHandler"
      class="income-form"
    >
      <select
      ...
      <input
        ...
        placeholder="Сумма для откладывания"
        type="number"
        class="input-field"
      />
      <button
    ...
    <ul class="income-list">
      ...
      >
        ...
        <button
          @click="removeIncomeHandler(income.id)"
          class="delete-button"
        >
          Удалить
        </button>
    ...
    const addIncomeHandler = async () => {
      if (incomeAmount.value > 0) {
        ...
        if (
          ...
          allocationAmount.value <= incomeAmount.value
        ) {
